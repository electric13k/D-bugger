...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  return Response.json({ state: row ? JSON.parse(row.state_json) : null, updatedAt: row?.updated_at || null }, { headers: { 'Cache-Control': 'no-store', Vary: 'Cookie' } });
};

export const onRequestPut: PagesFunction<Env> = async ({ request, env }) => {
  const body = await request.json<{ state?: unknown }>();
  if (!body.state) return Response.json({ error: 'state is required' }, { status: 400, headers: { 'Cache-Control': 'no-store', Vary: 'Cookie' } });
  const now = Date.now();
  ...
  return Response.json({ ok: true, updatedAt: now }, { headers: { 'Cache-Control': 'no-store', Vary: 'Cookie' } });
};
